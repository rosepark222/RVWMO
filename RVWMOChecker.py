...
class RVWMOChecker:
    ...
    def _compute_temp_edges_per_byte(self, load, rf_map):
        edges = []
        for byte, store in rf_map.items():
            edges.append((store.order, load.order, RF))
        return edges

    def _compute_new_seen_per_byte(self, rf_map, seen, load_hart):
        new_seen = dict(seen)
        for store in rf_map.values():
            if store.order > new_seen.get(store.hart, 0):
                new_seen[store.hart] = store.order
        return new_seen
    # =============================================================================
    # Recursive RF/CO search (HOT PATH)
    # =============================================================================
    def _search_recursive(self, load, load_value, tbd, partial_co, unplaced, seen,
                        rf_source, best_result, strict_seen=False):
        """
        Correct RVWMO behavior:
        - Loads may merge bytes from multiple stores.
        - RF is per-byte (not per-store).
        """

        # BASE CASE: no unplaced stores left -> check if all bytes resolved
        if not unplaced:
            if tbd != 0:
                return best_result  # unresolved bits -> reject

            # No strict_seen filtering here — RVWMO allows merging.
            # Only need to check for cycles.
            temp_edges = self._compute_temp_edges_per_byte(load, rf_source)
            if self._creates_cycle(temp_edges):
                return best_result

            new_seen = self._compute_new_seen_per_byte(rf_source, seen, load.hart)
        ...
        for i, store in enumerate(unplaced):
            ...
            # Compute byte contribution mask
            contribution = (load_value ^ store.value) & tbd & store.mask
            value_matches = ((contribution & store.mask) == 0)

            new_rf_source = dict(rf_source) if rf_source else {}
            
            # If store contributes bytes, record them per-byte
            if value_matches:
                # For every byte this store covers, assign it
                for byte in range(8):
                    byte_mask = (0xFF << (8*byte))
                    if store.mask & byte_mask:
                        if (load_value & byte_mask) == (store.value & byte_mask):
                            new_rf_source[byte] = store

            # Update unresolved bits
            ...
            result = self._search_recursive(
                ...
                load_value=load_value,
                tbd=new_tbd,
                partial_co=new_partial_co,
                unplaced=new_unplaced,
                seen=seen,
                rf_source=new_rf_source,
    ...
    def _search_recursive_buggy(self, load, load_value, tbd, partial_co, unplaced,
